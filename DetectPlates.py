...
def detectPlatesInScene(imgOriginalScene):
    ...
    print("\n" + str(len(listOfPossiblePlates)) + " number of possible plates found in image.")  # 13 with MCLRNF1 image
    ...
    if Main.showSteps == True: # show steps #######################################################
        ...
        for i in range(0, len(listOfPossiblePlates)):
            ...
            cv2.line(imgContours, tuple(p2fRectPoints[0].astype(int)), tuple(p2fRectPoints[1].astype(int)), Main.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[1].astype(int)), tuple(p2fRectPoints[2].astype(int)), Main.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[2].astype(int)), tuple(p2fRectPoints[3].astype(int)), Main.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[3].astype(int)), tuple(p2fRectPoints[0].astype(int)), Main.SCALAR_RED, 2)
        ...
        print("\nNumber plate detection complete, click on any image and press a key to begin char recognition . . .\n")
        cv2.waitKey(0)
    # end if # show steps #########################################################################
